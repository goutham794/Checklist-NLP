...
def eval_fn(data_loader: DataLoader, model):
    model.eval()
    fin_targets = []
    fin_outputs = []
    with torch.no_grad():
...
def fit(num_epochs: int, model, loss_fn, opt, train_dl: DataLoader, valid_dl: DataLoader):
    """
    Function for training the model for given number of epochs.
    """
